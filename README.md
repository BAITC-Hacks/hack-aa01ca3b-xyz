...
**Скорость.** Проверено на MacBook с Apple M-series и 8 ГБ ОЗУ. Двухминутная запись распознаётся с диаризацией примерно за 95 с (MPS). Анализ ИИ-агентом на qwen3:4b занимает ещё около 1–2 минут. На CPU без GPU обработка идёт медленнее, для быстрой проверки подойдёт `AGENT_VERIFY=0`.
