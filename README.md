...
| Поручения с ответственным и сроком | Локальный ИИ-агент (Ollama): участники → поручения → самопроверка → проверка дат | `processing._local_llm_analysis` | блок «ПОРУЧЕНИЯ», `python scripts/evaluate.py` |
...
4. **Python-проверка.** Срок раньше даты совещания сбрасывается, дубли убираются. Поручения без срока попадают во флаги. Каждому поручению подставляется время реплики для прослушивания.
...
| `WHISPER_MODEL` | `small` | Запасной faster-whisper, если модели kaz-rus не скачаны |
