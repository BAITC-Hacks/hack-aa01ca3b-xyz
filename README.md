...
**Что дальше:**
- реестр поручений по всем совещаниям с памятью между встречами. Концепт — макет панели контроля сроков `frontend/index.html`;
...
```
app/
├── api/                         зарезервировано под HTTP API; FastAPI пока не реализован
├── core/                        общие пути и конфигурация приложения
└── services/
...
app/main.py                      основной интерфейс Streamlit и точка запуска
data/
├── samples/                     тестовые записи, примеры кейса, эталон gold.json
└── templates/fonts/             шрифты для PDF; сами документы формируются кодом
frontend/                        статический макет панели (index.html, app.js, styles.css)
...
Структура организована по целевым каталогам кейса. Текущий прототип использует Streamlit и вызывает сервисы напрямую; REST API на FastAPI, React-клиент, `Dockerfile` и `docker-compose.yml` в этой версии не реализованы. Их не следует считать готовыми только из-за наличия целевых каталогов в первоначальном эскизе.
